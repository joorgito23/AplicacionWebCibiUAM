      ...
      <div class="reservas-grid">
        <div v-for="r in reservas" :key="r.id" v-show="new Date(r.fechaInicio.replace(' ', 'T')) >= new Date() &&
          ...
          <div class="card-header">
            ...
            <span :class="['status-badge', getEstadoClass(r)]">
              {{ getEstadoLabel(r) }}
            </span>
          </div>
          ...
          <div class="card-footer">
            <button class="btn-cancelar" @click="cancelar(r.id)" :data-cy="'boton-' + r.id">
              <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"
                stroke-linecap="round" stroke-linejoin="round">
...
.reserva-card.encurso .card-accent {
  background: #16a34a;
}

.reserva-card.pendiente .card-accent {
  background: #2563eb;
}

.reserva-card.cancelada .card-accent {
  background: #dc2626;
}

.reserva-card.finalizada .card-accent {
  background: #9ca3af;
}
